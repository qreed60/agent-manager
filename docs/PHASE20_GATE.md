...
## Final Validation Modes

`scripts/validate_agent_run.py` distinguishes two nightly approval contexts:

- Default no-OpenHands nightly mode: human approval packets must deny model
  calls, OpenHands, source writes, PR creation, push, merge, auto-apply,
  cleanup, and code-writing tasks.
- Explicitly gated Phase 20 overnight OpenHands mode: a human approval packet
  may report the single Phase 20 code-writing task only when the latest nightly
  window contains `OVERNIGHT_OPENHANDS_WRITE_SUMMARY.json` generated by
  `phase20_first_overnight_write_capable_run` and that summary proves the run
  stayed bounded to one task, at most one retry, check-only apply mode,
  `applied: false`, a clean canonical repo, changed files within the allowed
  list, and no apply, commit, push, merge, PR creation, or cleanup.

The Phase 20 exception does not permit canonical source writes, auto-apply,
commit, push, merge, PR creation, or worktree cleanup. Passing summaries must
recommend `accept_for_manual_review` with a passing apply check. Blocked
summaries must remain safe and include a recommended human action.

If a later default nightly run moves `latest_nightly_window` back to
no-OpenHands mode before the human approval packet is regenerated, the final
validator may use the recorded Phase 20 nightly summary from run history for
that single bounded code-writing count. A normal approval packet with no safe
Phase 20 summary still fails.
